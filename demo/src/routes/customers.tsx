...
function CustomersPage() {
  ...
  const dueSeverityClass = (dueAmount: number) => {
    if (dueAmount >= 100000) return 'bg-rose-100 text-rose-700'
    if (dueAmount >= 25000) return 'bg-orange-100 text-orange-700'
    ...
    <div className="w-full space-y-8 px-4 pb-10 pt-4 md:px-6">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
          ...
          <Metric label="Last" value={latestDate(summary.lastBillDate, summary.lastPaymentDate) || '-'} />
        </div>
      </section>
            ...
            <table className="w-full min-w-[980px]">
              <thead>
                <tr className="bg-slate-50">
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Customer</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Opening</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Billed</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Paid</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Net</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Status</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Last Activity</th>
                ...
                {filteredRows.map((row, index) => (
                  <tr key={row.customer.id} className={`border-t border-slate-100 transition hover:bg-slate-50 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                    <td className="px-3 py-3">
                      <p className="text-sm font-medium text-slate-800">{row.customer.name}</p>
                      {row.customer.gstin && <p className="text-xs text-slate-500">GSTIN: {row.customer.gstin}</p>}
                      {row.customer.phone && <p className="text-xs text-slate-500">Phone: {row.customer.phone}</p>}
                    </td>
                    <td className="px-3 py-3 text-right font-mono text-sm text-slate-800">{formatInrInteger(row.openingBalance)}</td>
                    <td className="px-3 py-3 text-right font-mono text-sm text-slate-800">{formatInrInteger(row.billedTotal)}</td>
                    <td className="px-3 py-3 text-right font-mono text-sm text-slate-800">{formatInrInteger(row.paidTotal)}</td>
                    <td className="px-3 py-3">
                      <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${row.balanceLabel === 'Due' ? dueSeverityClass(row.dueAmount) : row.balanceLabel === 'Advance' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>
                        {row.balanceLabel}: {formatInrInteger(Math.abs(row.netBalance))}
                      </span>
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-xs text-slate-600">
                      <p><span className="text-slate-500">Latest:</span> <span className="font-semibold text-slate-700">{latestDate(row.lastBillDate, row.lastPaymentDate) || '-'}</span></p>
                      <p className="mt-0.5"><span className="text-slate-500">Bill:</span> <span className="font-medium">{row.lastBillDate || '-'}</span></p>
                      <p className="mt-0.5"><span className="text-slate-500">Payment:</span> <span className="font-medium">{row.lastPaymentDate || '-'}</span></p>
                    </td>
