...
export function buildPartyRows(params: {
  ...
  return params.customers
    .map((customer) => {
      ...
      const totalGst = bills.reduce((sum, bill) => {
        const base = itemSumByBill.get(bill.id) ?? 0
        return sum + (bill.gstAmount && bill.gstAmount > 0 ? bill.gstAmount : (base * (bill.gstRate ?? 0)) / 100)
      }, 0)
      const billedTotal = bills.reduce(
        (sum, bill) => sum + calculateBillTotalFromBase(itemSumByBill.get(bill.id) ?? 0, bill.transport ?? 0, bill.gstRate ?? 0, bill.gstAmount),
        0,
      )
...
export function buildPartyEvents(params: {
  ...
  payments: Array<{ id: string; businessDate?: string; date?: string; createdAt?: string; amount: number; mode: string; note?: string }>
}): PartyEvent[] {
  ...
  for (const bill of params.bills) {
    timeline.push({
      ...
      sortOrder: 1,
      type: 'Bill',
      details: bill.details ?? `Bill ${bill.bookNo}/${bill.billNo}`,
  ...
  for (const payment of params.payments) {
    timeline.push({
      ...
      compactDetails: payment.note ? `Note: ${payment.note}` : undefined,
      debit: 0,
  ...
  return timeline.map((entry) => {
    ...
    return {
      ...
      compactDetails: entry.compactDetails,
      debit: entry.debit,
      credit: entry.credit,
