...
async function saveMarketRateDayRecord(rate: number, rateDate: string) {
  ...
  const existing = await pb.collection('brass_rates').getFirstListItem(`date = "${day}"`).catch(() => null)
  if (existing) {
...
async function loadMarketRateDayRecord(dateIso: string) {
  try {
    const record = await pb.collection('brass_rates').getFirstListItem(`date = "${dateIso}"`)
