...
describe('customers domain', () => {
  it('computes due amount when net balance is positive', () => {
    const summaries = buildCustomerLedgerSummaries({
      customers: [{ id: 'c1', companyName: 'Alpha Metals', name: 'Alpha', active: true, openingBalance: 200 }],
  ...
  it('computes advance when overpaid', () => {
    const summaries = buildCustomerLedgerSummaries({
      customers: [{ id: 'c1', companyName: 'Beta Industries', name: 'Beta', active: true, openingBalance: 0 }],
  ...
  it('returns clear label when perfectly settled', () => {
    const summaries = buildCustomerLedgerSummaries({
      customers: [{ id: 'c1', companyName: 'Gamma Works', name: 'Gamma', active: true, openingBalance: 100 }],
      bills: [],
      billItems: [],
